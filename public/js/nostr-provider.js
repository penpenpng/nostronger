...
function waitReport(device, createListener, timeout) {
...
//   })
// }
async function getNextStrain(device) {
    return waitReport(device, (resolve) => (event) => {
        const strain = getStrain(event);
        if (strain) {
            resolve(strain);
        }
